 ...
 */
const AuthForm = ({ preventDefaultAction, setActive, handleRegisterSubmit, handleLoginSubmit }) => {
    ...
    <div className="auth-tab" onClick={e => e.stopPropagation()}>
      ...
      <div className={`tab-content ${activeTab === "register" ? "active" : ""}`}>
        <form className="form" onSubmit={e => preventDefaultAction(e)}>
          <Input required name="username" title="Username" type="text" {...username} />
          <Input required name="email" title="Email" type="email" {...email} />
          <Input required name="password" title="Password" type="password" {...password} />
          <div className="action-buttons-container">
            <Button type="cancel" text="Cancel" onClick={() => setActive(false)} />
            <Button onClick={registerSubmit} type="confirm" text={registerLoading ? <img className="spinner" src={LoadingSpinner}></img> : "Register"} />
          </div>
        </form>
      </div>
      <div className={`tab-content ${activeTab === "login" ? "active" : ""}`}>
        <form className="form" onSubmit={e => preventDefaultAction(e)}>
          <Input required name="login-email" title="Email" type="email" {...loginEmail} />
          <Input required name="login-password" title="Password" type="password" {...loginPassword} />
          <div className="action-buttons-container">
            <Button type="cancel" text="Cancel" onClick={() => setActive(false)} />
            <Button onClick={loginSubmit} type="confirm" text={loginLoading ? <img className="spinner" src={LoadingSpinner}></img> : "login"} />
          </div>
        </form>
      </div>
    </div>
